...
func handleBotMsg(message whatsapp.TextMessage) {
	...
	for i := 0; i < len(commands); i++ {
		...
		if (len(commands[i].groups) == 0 || arrayContains(commands[i].groups, JidToGroupID(message.Info.RemoteJid))) && strings.HasPrefix(strings.Split(strings.ToLower(message.Text), " ")[0], strings.ToLower(commands[i].prefix)) {
